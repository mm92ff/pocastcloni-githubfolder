...
class ErrorResponseBodyLimitInterceptor(
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        return if (response.isSuccessful || originalBody == null) {
            response
        } else {
            val contentType = originalBody.contentType()
            val limitedBytes = readLimitedBytes(originalBody)
    ...
    private fun readLimitedBytes(originalBody: ResponseBody): ByteArray =
        try {
            val buffer = Buffer()
