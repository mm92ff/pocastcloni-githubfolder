...
interface PodcastRepository {
    ...
    suspend fun addPodcast(url: String, downloadLimit: Int, mode: FeedUpdateMode, sortOrder: Long? = null, forceFull: Boolean = false)
    ...
    // --- EPISODE ACTIONS ---
    suspend fun markEpisodePlayed(guid: String, played: Boolean, datePlayed: Date?)
    suspend fun toggleEpisodePlayed(episode: EpisodeEntity)
    suspend fun savePlaybackProgress(guid: String, positionMs: Long)
    suspend fun markAllAsSeen()
    suspend fun cleanupPlayedEpisodes()
    suspend fun updatePodcastSettings(podcast: Podcast, autoDownloadEnabled: Boolean)
    ...
    suspend fun updatePodcastNewFlag(rssUrl: String, hasNew: Boolean)
