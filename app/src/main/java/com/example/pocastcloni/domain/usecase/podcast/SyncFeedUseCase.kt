...
constructor(
    ...
    private val feedSyncPersistence: FeedSyncPersistence,
    ...
    private suspend fun syncSmart(
        ...
        if (response.code() == HttpURLConnection.HTTP_NOT_MODIFIED) {
            existing?.let {
                val autoDownloadEnabled = feedSyncPersistence.touchLastRefreshed(url, Date())
                if (autoDownloadEnabled) triggerAutoDownloads(url, downloadLimit)
            }
            return
        }
    ...
    private suspend fun processParsedData(
        ...
        val podcastFeedUpdate = PodcastFeedUpdate(
            rssUrl = url,
            title = title,
        ...
        val newPodcast = if (existing == null) {
            PodcastEntity(
                rssUrl = url,
                title = title,
                description = description?.stripHtml() ?: "",
        ...
        val autoDownloadEnabled = feedSyncPersistence.persistFeedUpdate(
