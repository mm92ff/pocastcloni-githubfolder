...
class AudioPlayerControllerConcurrencyTest {
    ...
    fun `delayed episode mapping cannot overwrite the current episode ui`() = runTest {
        ...
        coEvery { fixture.repository.getEpisode(FIRST_EPISODE_ID) } coAnswers {
            firstReadStarted.complete(Unit)
            releaseFirstRead.await()
            episode(FIRST_EPISODE_ID)
        }
        coEvery { fixture.repository.getEpisode(SECOND_EPISODE_ID) } returns episode(SECOND_EPISODE_ID)
    ...
    private class Fixture(
        ...
        init {
            ...
            coEvery { repository.getEpisode(any()) } returns null
            ...
            every { mapper.mapToMediaItem(any(), any(), any()) } answers {
                MediaItem.Builder()
                    .setMediaId(firstArg<EpisodeEntity>().episodeId.toString())
                    .build()
            }
            every { mapper.mapToUiState(any(), any(), any(), any()) } answers {
                val mappedEpisode = secondArg<EpisodeEntity?>()
    ...
    private companion object {
        ...
        private fun episode(episodeId: Long): EpisodeEntity =
            EpisodeEntity(
                guid = "guid-$episodeId",
